...
const dbFunctions = {
  ...
  deleteToken: async function (req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: `Token id:${req.id} was deleted succesfully` });
    } catch (err) {
  ...
  getAdById: async function (req) {
    ...
    const payload = {
      id: res[0].id,
      name: res[0].nev,
      description: res[0].leiras,
      category: res[0].kategoria,
      price: res[0].ar,
      countyId: res[0].varmegyeId,
      settlement: res[0].telepules,
      ownerId: res[0].tulajId,
      date: res[0].datum,
    };
    return payload;
  },
